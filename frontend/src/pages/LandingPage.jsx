      ...
      <div className="container">
        ...
        {!loading && services.length > 0 && (
          <p style={{ textAlign: 'center', marginTop: '32px', fontSize: '.8rem', color: 'var(--text-faint)' }}>
            ...
            <button onClick={() => window.open('https://wa.me/5599981763335', '_blank')} style={{ background: 'none', border: 'none', color: 'var(--blue)', cursor: 'pointer', fontWeight: 600, fontSize: '.8rem', fontFamily: 'inherit' }}>Fale conosco →</button>
          </p>
        )}
      </div>
    </section>
    ...
    try {
      ...
      const res = await fetch(API + '/agendamentos', {
        ...
        body: JSON.stringify({ 
          ...
          servico_id: parseInt(servico.id) || 1, 
          data_hora, 
